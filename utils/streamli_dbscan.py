...
st.title("🔬 STL Segmentation Outlier Explorer")

# Load your specific JSON
try:
    ...
    df = df[features].copy()
    df['Mesh_volume_mm3'] = df['Mesh_volume_mm3'] / 1000  # mm3 to cm3
    df['Surface_Area_mm2'] = df['Surface_Area_mm2'] / 100  # mm2 to cm2
    # Rename for easier coding
    df.rename(columns={'Mesh_volume_mm3': 'volume', 'Surface_Area_mm2': 'surface_area'}, inplace=True)
...
eps = st.sidebar.slider("Epsilon (Strictness)", 0.001, 0.3, 0.035, step=0.001, help="Lower = stricter", format="%.3f") 
...
# Calculate zoom ranges using the renamed columns 'volume' and 'surface_area'
if search_id:
    ...
    st.sidebar.success(f"Focused on {search_id}")
else:
...
fig.update_layout(
    ...
    height=700, xaxis_title="Volume (cm³)",
